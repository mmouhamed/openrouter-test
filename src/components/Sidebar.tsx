...
const getNavigationItems = (_isUserAdmin: boolean): SidebarItem[] => {
  const items: SidebarItem[] = [
    ...
    { 
      ...
      icon: '🎨',
      badge: 'New',
      description: 'Modern ChatGPT-style interface'
    }
  ];
            ...
            return (
              <Link
                ...
                className={`flex items-center space-x-2 sm:space-x-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl transition-all duration-200 touch-manipulation ${
                  isActive
                    ? 'bg-gradient-to-r from-teal-500/20 to-cyan-500/20 dark:from-teal-500/30 dark:to-cyan-500/30 text-teal-600 dark:text-teal-400 shadow-sm'
                    : item.comingSoon
              ...
              >
                ...
                {item.badge && (
                  <span className="ml-auto px-2 py-0.5 text-xs font-semibold bg-gradient-to-r from-teal-600 to-cyan-600 text-white rounded-full shadow-sm">
                    {item.badge}
                  </span>
                )}
              </Link>
          ...
          {user ? (
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2 min-w-0 flex-1">
                <div className="w-7 sm:w-8 h-7 sm:h-8 bg-gradient-to-r from-teal-500 to-cyan-500 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm">
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 text-white font-semibold py-2.5 px-4 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md"
            >
              Sign In
            </button>
          )}
